...
inSilicoSCRNASeq <- function(CellModels, ncells=96, pathways=NULL, ReferenceDataSet=NULL, fasta=NULL,
                             samplingFrequency=1, combineFUN="max", attrsep = " ", ...){
  
  # call standard function to simulate mean expression values for all pathways
  simMeanExprs <- simulateMeanSCExpression(CellModels=CellModels, pathways=pathways, ncells=ncells,
  ...
  if (is.null(fasta)) {
    # simulate single cell data with a negative binomial error model
    ...
    # run polyester
    message('Simulating time-course single cell RNA-seq data with Polyester')
