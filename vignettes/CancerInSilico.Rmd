...
```{r}
simple_mod <- suppressMessages(inSilicoCellModel(initialNum=30, runTime=72, density=0.1, outputIncrement=24, randSeed=123))
```
...
and returns a 1 if the cell is currently in mitosis and 0 otherwise.
...
geneMeans <- rexp(length(allGenes), 1/20)
...
gene_exp <- inSilicoGeneExpression(simple_mod, pwys, params)
```
...
expression data is based on. This allows us to see exactly how our
...
property that is specific to off-lattice cell models. Now we can see a sharp
drop in mitosis activity near the end of the model.
...
acticvity - allowing for a smooth normalization.
...
```{r}
drug <- new("Drug", name="Drug_A", timeAdded=24, cycleLengthEffect=function(type, length) length * 2)
drug_mod <- suppressMessages(inSilicoCellModel(initialNum=30, runTime=72, density=0.1, drugs=c(drug), outputIncrement=24, randSeed=123))
...
plot(times, nCells, type="l", xlab="hour", ylab="number of cells")
lines(times, nCells_drug, type="l", xlab="hour", ylab="number of cells", col="red")
```
...
fast_cells_mod <- suppressMessages(inSilicoCellModel(initialNum=30, runTime=72, density=0.1, cellTypes=c(type_A), outputIncrement=24, randSeed=123))
...
plot(times, nCells, type="l", xlab="hour", ylab="number of cells")
lines(times, nCells_fast, type="l", xlab="hour", ylab="number of cells", col="red")
```
...
```{r}
type_B <- new("CellType", name="B", size=1, minCycle=16, cycleLength=function() 16 + rexp(1,1/4))
type_C <- new("CellType", name="C", size=1, minCycle=32, cycleLength=function() 32 + rexp(1,1/4))
two_types_mod <- suppressMessages(inSilicoCellModel(initialNum=30, runTime=72, density=0.1, cellTypes=c(type_B, type_C), cellTypeInitFreq=c(0.4,0.6), outputIncrement=24, randSeed=123))
```
...
# pathway behavior
pwy_B <- new("Pathway", genes=B_genes, expressionScale=function(model, cell, time) ifelse(getCellType(model, time, cell) == 1, 1, 0))
pwy_C <- new("Pathway", genes=C_genes, expressionScale=function(model, cell, time) ifelse(getCellType(model, time, cell) == 2, 1, 0))

# calibrate pathways
geneMeans <- rexp(length(c(B_genes, C_genes)), 1/20)
data <- t(pmax(sapply(geneMeans, rnorm, n=25, sd=2), 0))
...
case we set *RNAseq* and *singleCell* to be true, and then we also must pass
a *splatParams* object since *CancerInSilico* user *splatter* to generate
single cell data.
...
ge <- inSilicoGeneExpression(two_types_mod, c(pwy_B, pwy_C), params)
```

## Visualize Data

We can visualize the single cell plots with the *Rtsne* package.

```{r}
cells <- unname(sapply(colnames(ge$expression), function(x) strsplit(x,"_")[[1]][1]))
cells <- as.numeric(gsub("c", "", cells))
type <- sapply(cells, getCellType, model=two_types_mod, time=two_types_mod@runTime)

tsne_out <- Rtsne(t(ge$expression), dims=3, initial_dims=3, perplexity=5, theta=0.5, pca=TRUE, max_iter=500, verbose=TRUE)
open3d()
