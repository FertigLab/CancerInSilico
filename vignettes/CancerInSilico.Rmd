...
The plots are colored according to cell phase - cells in interphase are colored
gray and cells in mitosis are colored black.
...
heatmap.2(gene_exp$expression[!ndx,], 
    ...
    RowSideColors = ifelse(rownames(gene_exp$expression[!ndx,]) %in%
...
params@splatParams <- splatter::setParam(params@splatParams, "dropout.present",
    TRUE)
...
```{r}
cells <- unname(sapply(colnames(ge$expression), function(x)
    strsplit(x,"_")[[1]][1]))
cells <- as.numeric(gsub("c", "", cells))
type <- sapply(cells, getCellType, model=two_types_mod,
    time=two_types_mod@runTime)

tsne_out <- Rtsne(t(ge$expression), dims=3, initial_dims=3, perplexity=5,
    theta=0.5, pca=TRUE, max_iter=500, verbose=TRUE)
open3d()
