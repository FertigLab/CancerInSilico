...
The main objective of the *CellModel* package is to provide a single, consistent R interface for running several different types of cell-based models [see @SZABO2013 for a review of current approaches to cell-based modeling]. While most cell-based models appear similiar on the surface, there can be significant variety in the underlying cell mechanics of the model. This results in a difference both in terms of output and efficiency. This package aims to provide the user with an easy way to both use and compare each type of model. The currrent version of *CellModel* contains a basic, off-lattice model of cell growth. This model is based off the work in [@DRASDO2003]. It was chosen to be implemented first since it is a basic model that provides enough infastructure to test some simple ideas related to drug response and resistance.
...
This is a model specific parameter. For a more detailed explanation [see @Drasdo]. Note that the effective energy F_T is set to 1 is this version of the model.
...
den <- c()
for (i in 0:72) {
  den[i+1] <- getDensity(simple_run,i)
}
...
In this simulation, we assume the drug does not have a binary effect on the cycle time, but rather effects the cycle according to some distribution. The question here would be how long it takes for a significant change in the distribution to occur.

```{r}
set.seed(0)
cycle_dist <- 8 + 16 * rbeta(10000,2,4)
adv_case <- runModel(100,200,0.001,cycleLengthDist=cycle_dist,outputIncrement=120,inheritGrowth=T)
```

Note that in order to reproduce results exactly, it is neccesary to set the seed before generating *cycle_dist* since *randSeed* only sets the seed internal to the model.

```{r}
plot(density(getCycleLengthDistribution(adv_case,1)))
plot(density(getCycleLengthDistribution(adv_case,200)))

mean(getCycleLengthDistribution(adv_case,1))
mean(getCycleLengthDistribution(adv_case,200))
```

Here we notice that at time __ we finally saw a significant change in the distribution.

This general framework allows for detailed analysis on the effect of heritable changes in the "growth rate" of a cell.
