...
```{r,eval=FALSE}
single_cell_run <- runCancerSim(initialNum = 1,
                                runTime = 84,
                                density = 0.01,
                                cycleLengthDist = 12,
                                inheritGrowth = FALSE,
...
The output of the model is stored in a *CellModel* object. This object has two fields: *cells* and *parameters*. *parameters* contains a list of all the parameters used to run the model. *cells* contains information about the cell population at every time step in the model. The primary purpose of this object is to store all the information about a run in a single place and provide a standard data structure for the package functions to handle. Since this is just raw data, users should never deal with this object directly (see code below).
...
First we need a *CellModel* object as an example.
