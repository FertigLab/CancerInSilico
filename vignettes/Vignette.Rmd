...
There is one exception as the "plotTotalCells" function is the only one that requires one argument, the "model" argument.
...
Through the visualization the total cells over time, comparisons can be made between the density and growth rates which are received from other functions at the respective time. Within the example, the growth of a model with an initial number of one hundred cells, was plotted over a hundred timesteps.
![](plotTotalCells.png)

Above is a graph depicting the change of amount of cells over the time index. The general trend of the plot should show a near exponential growth on the amount of cells in the model.
