...
```{r,eval=FALSE}
simulateGeneExpGroup(model,pathway,perError = 0.1,opt = 1)
```
...
```{r}
genes_per_cell_expressed = simulateGeneExpSing(data, pathways, opt = 2)
...
```{r,eval=FALSE}
simulateGeneExpSing(model,pathway,perError = 0.1,opt = 1)
```
...
The differences between the *suffix* variants of each function used for calculations are described in their respective "**return**" and "**examples**" sections.

### *simulateGToMPath*

This is the function that generates a simulation for the G<sub>2</sub> to Mitosis pathway.
...
This is the function that generates a simulation for the G<sub>1</sub> to Synthesis pathway.
...
This is the function that generates a simulation for the Contact Inhibition pathway.
