...
# inSilico RNA Gene Expression Sequencing

Within this section of the package, two functions are used to generate the gene expression distribution data. These functions are used to take all the given models provided by a user, and merge them based on the model given such as max, sum or a user-defined function. This allows for the output to be varied depending on the data the user wants to view.

## *inSilicoSCRNASeq*: gene expression of individual cells from RNA Seq Data



## *inSilicoRNASeq*: gene expression from RNA Seq Data
...
*ncells* - [Integer] Optional Parameter; Value greater than or equal to 0; Only for *inSilicoSCRNASeq*
*pathways* - [List] Optional Parameter;
*referenceDataset* - 
*lambda* - 
*fasta* - 
*samplingFrequency* - 
...
# Gene Summation Functions *simulateGeneExpGroup* and *simulateGeneExpSing*
...
```{r}
average_genes_expressed = simulateGeneExpGroup(data, pathways, perError = 0.01, opt = 1, sampFreq = 1, ncell = 0)
...
```{r,eval=FALSE}
simulateGeneExpGroup(model,pathway,perError = 0.1,opt = 1, sampFreq = 1, ncell = 0)
```
...
```{r}
genes_per_cell_expressed = simulateGeneExpSing(data, pathways, opt = 2, sampFreq = 1, ncell = 0)
...
```{r,eval=FALSE}
simulateGeneExpSing(model,pathway,perError = 0.1,opt = 1, sampFreq = 1, ncell = 0)
```
...
*sampFreq* - [Double] Optional Parameter; Any value between model runtime and 0.  
*ncell* - [Integer] Optional Parameter; Any value
...
The option parameter that allows the user to decide on the output's noise model. The first option, with a default of 1, is the normal noise model based on the means of the genes at each gene. The second option is a negative binomial noise model with the calculations based on the *limma::voom* model. The third option will be an alternate negative binomial model with calculations based on the *polyester* package.

*sampFreq*

The parameter specifying the frequency the data from the model would be collected. This is all based on the model hour, where a decimal will be that part of an hour in real time.

*ncell*

The number of cells randomly collected at any given timepoint. Value of "0" sets it to not take any sample.
